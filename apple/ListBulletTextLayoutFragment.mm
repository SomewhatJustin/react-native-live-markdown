...
@implementation ListBulletTextLayoutFragment

- (void)drawAtPoint:(CGPoint)point inContext:(CGContextRef)ctx {
  // Draw the bullet point character
  ...
  // Use the syntax color for the bullet
  UIColor *bulletColor = _markdownUtils.markdownStyle.syntaxColor;

  // Create attributed string with bullet character
  NSString *bullet = @"\u2022"; // bullet point character
  NSDictionary *attrs = @{
    NSFontAttributeName: font,
    NSForegroundColorAttributeName: bulletColor
  };
  NSAttributedString *bulletString = [[NSAttributedString alloc] initWithString:bullet attributes:attrs];
  ...
  [bulletString drawAtPoint:CGPointMake(x, y)];
